...
export function InputForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-[35rem]">
      ...
      <textarea id="useCase" value={useCase} onChange={event => setUseCase(event.target.value)} placeholder="Optional: what are you hoping this project can do?" className="mb-2 min-h-16 w-full rounded-md border border-[var(--gh-border)] bg-[var(--gh-canvas-inset)] px-3 py-2.5 text-sm text-[var(--gh-fg)] outline-none placeholder:text-[var(--gh-fg-subtle)] focus:border-[var(--gh-accent)] focus:ring-3 focus:ring-[var(--gh-accent-emphasis)]/25" />
      <label className="sr-only" htmlFor="githubUrl">GitHub repository URL</label>
      <div className="flex flex-col gap-2 sm:flex-row">
        <div className="flex h-11 min-w-0 flex-1 items-center rounded-md border border-[var(--gh-border)] bg-[var(--gh-canvas-inset)] transition-colors focus-within:border-[var(--gh-accent)] focus-within:ring-3 focus-within:ring-[var(--gh-accent-emphasis)]/25">
          <svg aria-hidden="true" className="ml-3 shrink-0 text-[var(--gh-fg-subtle)]" width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M2 2.5A2.5 2.5 0 0 1 4.5 0h8.75a.75.75 0 0 1 .75.75v12.5a.75.75 0 0 1-.75.75h-2.5a.75.75 0 0 1 0-1.5h1.75v-2h-8a1 1 0 0 0-.714 1.7.75.75 0 1 1-1.072 1.05A2.495 2.495 0 0 1 2 11.5Zm10.5-1h-8a1 1 0 0 0-1 1v6.708A2.486 2.486 0 0 1 4.5 9h8Z" /></svg>
          <input ref={inputRef} id="githubUrl" type="text" required value={githubUrl} onChange={event => { setGithubUrl(event.target.value); if (searchResults) { setSearchResults(null); setActiveIndex(-1) } }} onKeyDown={handleInputKeyDown} placeholder="Paste a GitHub URL — or describe a project to search" autoComplete="off" role="combobox" aria-expanded={Boolean(searchResults?.length)} aria-controls="repo-search-results" className="min-w-0 flex-1 bg-transparent px-3 font-mono text-sm text-[var(--gh-fg)] outline-none placeholder:text-[var(--gh-fg-subtle)]" />
        </div>
        <button type="submit" disabled={loading || searching || !githubUrl.trim()} className="inline-flex h-11 items-center justify-center rounded-md border border-white/10 bg-[var(--gh-success-emphasis)] px-4 text-sm font-semibold text-white transition-colors hover:bg-[var(--gh-success-hover)] disabled:pointer-events-none disabled:opacity-50">
          {loading ? 'Creating preview…' : searching ? 'Searching…' : isRepoUrl || !githubUrl.trim() ? 'Try it' : 'Search'}
        </button>
      </div>

      {searchResults !== null && (
        <div id="repo-search-results" role="listbox" aria-label="Repository search results" className="mt-2 overflow-hidden rounded-md border border-[var(--gh-border)] bg-[var(--gh-surface)]">
      ...
      <section aria-labelledby="examples-heading" className="mt-7">
        ...
        <div className="overflow-hidden rounded-md border border-[var(--gh-border)] bg-[var(--gh-surface)]">
          {examples.map(example => (
            <button key={example.url} type="button" onClick={() => setGithubUrl(example.url)} className="group flex w-full items-center gap-3 border-b border-[var(--gh-border-muted)] px-3 py-2.5 text-left last:border-b-0 hover:bg-[var(--gh-surface-2)] focus-visible:relative focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-[var(--gh-accent)]">
              <span aria-hidden="true" className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: example.color }} />
              <span className="min-w-0 truncate font-mono text-[13px] text-[var(--gh-accent)]"><span className="text-[var(--gh-fg-subtle)]">{example.owner}/</span>{example.repo}</span>
              <span className="ml-auto shrink-0 text-xs text-[var(--gh-fg-subtle)]">{example.description}</span>
            </button>
          ))}
        </div>
      </section>
